...
###
# the period of the years 102 to 107
#        
###
# /tribe/102_107 query all tribes' variation info. of the indigenous population 
# 所有原住民族在各縣市之人口數變化'
#
@_ns_tribe.route('/102_107/')
class TribeList_102_107(Resource):
    ...
    def get(self):
        '''查詢各縣市(區域)內，每個原住民族人口的年度變動數量 (102年~107年)'''            
        ###
        # read the data between the year of 102 and 107
        #
        with open('..\\data\\population-var-102-107.csv', 'r', encoding='utf-8', newline='') as csvfile:
            df_102_107 = pd.read_csv(
                csvfile,
                header = 0,
                usecols = ['區域別',
                #usecols = ['日期區間', '區域別', '總計',
            ...
            df_102_107 = trim_all_cells(df_102_107) # trim whitespace from each cell in dataframe
            
            ###
            # transpose dataframe to coincide to the data model of response
            #
            df = df_102_107.T # transpose()
...
###
# /tribe/102_107/<string:tribe> query one specific tribe's variation info. of the indigenous population 
#
@_ns_tribe.route('/102_107/<string:tribe>')
@_ns_tribe.doc(responses={404: '找不到該原住民族的資料'}, params={'tribe': '原住民族族名'})
class Tribe_102_107(Resource):
    ...
    def post(self, tribe): #get(self, tribe):
        '''查詢某一個特定原住民族(如阿美族)，在各縣市(區域)人口的年度變動數量 (102年~107年)'''
        ###
        ...
        #
        df_102_107.rename({'區域別': 'area', tribe: 'amount'}, axis='columns', inplace=True) # axis=1
...
###
# /area/102_107 query the tribes' variation info. of the indigenous population in the area (city/county) 
# 各縣市(區域)內之原住民族人口數變化'
#
@_ns_area.route('/102_107/')
class AreaList_102_107(Resource):
    ...
    def get(self):
        '''查詢原住民族人口統計數據，在各縣市的年度變動數量 (102年~107年)'''            
        ###
        # read the data between the year of 102 and 107
        #
        with open('..\\data\\population-var-102-107.csv', 'r', encoding='utf-8', newline='') as csvfile:
            ...
            #
            df1 = df_102_107.to_dict('records')
...
###
# /area/102_107/<string:area> query the tribes' variation info. of the indigenous population in the specific area (city/county) 
#
@_ns_area.route('/102_107/<string:area>')
@_ns_area.doc(responses={404: '找不到該縣市的資料'}, params={'area': '縣市名稱'})
class Area_102_107(Resource):
    ...
    def post(self, area): #get(self, area):
        '''查詢某一個縣市(如新北市)內，各原住民族人口的年度變動數量 (102年~107年)'''
        ###
        ...
        with open('..\\data\\population-var-102-107.csv', 'r', encoding='utf-8', newline='') as csvfile:
            ...
            #             
            filter = df_102_107['區域別'] == area # filter context in terms of the name of the city/county
            # construct the columns of the output response structure
            y_102_107_col= ['阿美族', '泰雅族', '排灣族', '布農族', '魯凱族', '卑南族', '鄒族', '賽夏族',
                            '雅美族', '邵族', '噶瑪蘭族', '太魯閣族', '撒奇萊雅族', '賽德克族',
                            '拉阿魯哇族', '卡那卡那富族', '尚未申報']
            # construct the row of the output response structure
            y_102_107 = [df_102_107.loc[filter, '阿美族'].values[0],
                         df_102_107.loc[filter, '泰雅族'].values[0],
                         df_102_107.loc[filter, '排灣族'].values[0],
                         df_102_107.loc[filter, '布農族'].values[0],
                         df_102_107.loc[filter, '魯凱族'].values[0],
                         df_102_107.loc[filter, '卑南族'].values[0],
                         df_102_107.loc[filter, '鄒族'].values[0],
                         df_102_107.loc[filter, '賽夏族'].values[0],
                         df_102_107.loc[filter, '雅美族'].values[0],
                         df_102_107.loc[filter, '邵族'].values[0],
                         df_102_107.loc[filter, '噶瑪蘭族'].values[0],
                         df_102_107.loc[filter, '太魯閣族'].values[0],
                         df_102_107.loc[filter, '撒奇萊雅族'].values[0],
                         df_102_107.loc[filter, '賽德克族'].values[0],
                         df_102_107.loc[filter, '拉阿魯哇族'].values[0],
                         df_102_107.loc[filter, '卡那卡那富族'].values[0],
                         df_102_107.loc[filter, '尚未申報'].values[0]]
            ###
            ...
            # 
            dict = {"tribe": y_102_107_col, "amount": y_102_107}
