...
#
_ns_tribe = api.namespace('tribe', description='原住民族人口變化(100年-107年): 族')
_ns_area = api.namespace('area', description='原住民族人口變化(100年-107年): 縣市')
...
###
# /tribe/100_102 query all tribes' variation info. of the indigenous population 
# 所有原住民族在各縣市之人口數變化'
#
@_ns_tribe.route('/100_102/')
class TribeList_100_102(Resource):
    ...
    def get(self):
        '''查詢各縣市(區域)內，每個原住民族人口的年度變動數量 (100年~102年)'''            
        ###
        # read the data between the year of 100 and 102
        #
        with open('..\\data\\population-var-100-102.csv', 'r', encoding='utf-8', newline='') as csvfile:
...
###
# /tribe/100_102/<string:tribe> query one specific tribe's variation info. of the indigenous population 
#

###
# the period of the years 100 to 102
#        

@_ns_tribe.route('/100_102/<string:tribe>')
@_ns_tribe.doc(responses={404: '找不到該原住民族的資料'}, params={'tribe': '原住民族族名'})
class Tribe_100_102(Resource):
    ...
    def post(self, tribe): #get(self, tribe):
        '''查詢某一個特定原住民族(如阿美族)，在各縣市(區域)人口的年度變動數量 (100年~102年)'''
        ###
        ...
        #
        df_100_102.rename({'區域別': 'area', tribe: 'amount'}, axis='columns', inplace=True) # axis=1
...
###
# /area/100_102 query the tribes' variation info. of the indigenous population in the area (city/county) 
# 各縣市(區域)內之原住民族人口數變化'
#
@_ns_area.route('/100_102/')
class AreaList_100_102(Resource):
    ...
    def get(self):
        '''查詢原住民族人口統計數據，在各縣市的年度變動數量 (100年~102年)'''            
        ###
        # read the data between the year of 100 and 102
        #
        with open('..\\data\\population-var-100-102.csv', 'r', encoding='utf-8', newline='') as csvfile:
            ...
            #
            df1 = df_100_102.to_dict('records')
...
###
# /area/100_102/<string:area> query the tribes' variation info. of the indigenous population in the specific area (city/county) 
#
@_ns_area.route('/100_102/<string:area>')
@_ns_area.doc(responses={404: '找不到該縣市的資料'}, params={'area': '縣市名稱'})
class Area_100_102(Resource):
    ...
    def post(self, area): #get(self, area):
        '''查詢某一個縣市(如新北市)內，各原住民族人口的年度變動數量 (100年~102年)'''
        ###
        ...
        with open('..\\data\\population-var-100-102.csv', 'r', encoding='utf-8', newline='') as csvfile:
            ...
            #             
            filter = df_100_102['區域別'] == area # filter context in terms of the name of the city/county
            # construct the columns of the output response structure
            y_100_102_col= ['阿美族', '泰雅族', '排灣族', '布農族', '魯凱族', '卑南族', '鄒族', '賽夏族',
                            '雅美族', '邵族', '噶瑪蘭族', '太魯閣族', '撒奇萊雅族', '賽德克族',
                            '拉阿魯哇族', '卡那卡那富族', '尚未申報']
            ...
            # 
            dict = {"tribe": y_100_102_col, "amount": y_100_102}
